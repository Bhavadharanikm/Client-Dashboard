...
# TARGET_MONTHS is now derived dynamically from the data in main()
# so new months are picked up automatically without any code changes.
...
def main():
    ...
    # Dynamically build the set of target months from all data in the workbook.
    # This means adding a new month never requires touching this script.
    all_keys: set[str] = set()
    for rows in workbook.get("rowsByClientSlug", {}).values():
        for row in rows:
            all_keys.add(month_key(row["year"], row["month"]))
    ...
    for client in workbook.get("clients", []):
        ...
        roi_entries = {}
        for selected_key in target_keys:
